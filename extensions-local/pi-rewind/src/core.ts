...
const SHADOW_HEAD_REF = "refs/heads/pi-rewind-shadow";
const repoCache = new Map<string, RepoInfo | null>();

function shadowDirFor(root: string): string {
  const hash = createHash("sha256").update(root).digest("hex").slice(0, 16);
  return join(homedir(), ".pi", "agent", "pi-rewind", "shadow-git", hash);
}

function writeShadowExcludes(gitDir: string): void {
  const patterns = [...IGNORED_DIR_NAMES]
    .flatMap((name) => [name, `${name}/`])
    .join("\n");
  writeFileSync(join(gitDir, "info", "exclude"), `${patterns}\n`);
}
...
function run(
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      ...
      else reject(new Error(stderr || `${command} ${args[0]} failed (code ${code})`));
    });
...
async function resolveRepo(cwd: string): Promise<RepoInfo | null> {
  const cached = repoCache.get(cwd);
  if (cached !== undefined) return cached;

  const gitRoot = await run("git", ["rev-parse", "--show-toplevel"], cwd).catch(() => null);
  const jjRoot = await run("jj", ["root"], cwd).catch(() => null);
  const root = gitRoot ?? jjRoot;
  if (!root) {
    repoCache.set(cwd, null);
    return null;
  }

  const shadow = shadowDirFor(root);
  const gitDir = join(shadow, ".git");
  mkdirSync(shadow, { recursive: true });
  if (!existsSync(gitDir)) {
    await run("git", ["init", "--quiet", shadow], root);
    await run("git", ["--git-dir", gitDir, "symbolic-ref", "HEAD", SHADOW_HEAD_REF], root);
  }
  writeShadowExcludes(gitDir);

  const repo = { root, gitDir, source: gitRoot ? "git" as const : "jj" as const };
  repoCache.set(cwd, repo);
  repoCache.set(root, repo);
  return repo;
}

function repoArgs(repo: RepoInfo, args: string[]): string[] {
  return ["--git-dir", repo.gitDir, "--work-tree", repo.root, ...args];
}

async function usesShadowGit(root: string): Promise<boolean> {
  return (await resolveRepo(root)) !== null;
}

export async function getCurrentBranch(root: string): Promise<string> {
  const repo = await resolveRepo(root);
  if (!repo) return "unknown";
  if (repo.source === "jj") return "jj";
  return run("git", ["rev-parse", "--abbrev-ref", "HEAD"], repo.root).catch(() => "unknown");
}

async function rememberShadowSnapshot(root: string, commitSha: string, treeSha: string): Promise<void> {
  if (!(await usesShadowGit(root))) return;
  await git(`update-ref ${SHADOW_HEAD_REF} ${commitSha}`, root);
  await git(`read-tree ${treeSha}`, root);
}
...
export function git(
  ...
  return resolveRepo(cwd).then((repo) => {
    if (!repo) throw new Error(`No git or jj repository found from ${cwd}`);
    const args = repoArgs(repo, parseArgs(cmd));
    return run("git", args, repo.root, opts);
  });
}
...
export const isGitRepo = (cwd: string) =>
  resolveRepo(cwd).then((repo) => repo !== null).catch(() => false);

export const getRepoRoot = (cwd: string) =>
  resolveRepo(cwd).then((repo) => {
    if (!repo) throw new Error(`No git or jj repository found from ${cwd}`);
    return repo.root;
  });
...
export async function createCheckpoint(opts: CreateCheckpointOpts): Promise<CheckpointData> {
  ...
  try {
    ...
    await rememberShadowSnapshot(root, commitSha, worktreeTreeSha);
...
export async function restoreCheckpoint(root: string, cp: CheckpointData): Promise<void> {
  const shadow = await usesShadowGit(root);
  ...
  if (shadow) {
    const commitSha = await git(`rev-parse --verify ${REF_BASE}/${cp.id}`, root);
    await rememberShadowSnapshot(root, commitSha, cp.worktreeTreeSha);
  } else {
